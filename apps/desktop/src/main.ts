...
const APP_DISPLAY_NAME = isDevelopment ? "Atelier (Dev)" : "Atelier (Alpha)";
const APP_USER_MODEL_ID = "dev.julian0xff.atelier";
const USER_DATA_DIR_NAME = isDevelopment ? "atelier-dev" : "atelier";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Atelier (Dev)" : "Atelier (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Atelier failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Atelier ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Atelier (Alpha)` on Linux). This is
